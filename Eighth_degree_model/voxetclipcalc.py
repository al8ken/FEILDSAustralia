...
MODEL_DATASET = os.path.join(MODELDIR, 'Eighth_degree_model.vo')
...
resolution = 0.125        # grid cell size (we are assuming equal in x and y)
originX = 100.0         # longitude origin (cell centre!)
originY = -55.0         # latitude origin (cell centre!)
thickness = 301.25 * U.km # below surface
l_air = 55. * U.km      # above surface
NX = 521                # number of grid cells in longitude
NY = 481                # number of grid cells in latitude
n_cells_v = 285         # total number of cells in vertical direction (inc air)
...
def ResidualCalculation(reg_mask):
    ...
    G_DATA = os.path.join(DATADIR,'Final_BouguerTC_UC10K_8thdeg.nc')
